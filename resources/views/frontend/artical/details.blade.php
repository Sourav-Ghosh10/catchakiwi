                ...
                <div class="col-lg-8 col-md-8 col-sm-12">
                    <div class="left_searchresults">
                        ...
                        <div class="article_detailsbody">
                            @if($article->image)
                                <img src="{{ asset($article->image) }}" class="img-fluid rounded mb-4 w-100"
                                    alt="{{ $article->title }}">
                            @endif
                            ...
                            <div class="contribute_article">
                                ...
                                <a
                                    href="{{ url($country_name . '/profile/' . Crypt::encryptString($article->user->id)) }}">
                                    <img src="{{ $article->user->profile_image }}" alt="Profile"
                                        style="width: 50px; height: 50px; border-radius: 50%;">
                                </a>
                                <h4><a
                                        href="{{ url($country_name . '/profile/' . Crypt::encryptString($article->user->id)) }}">{{ $article->user->name }}</a>
                                </h4>
                                <p><strong>Posted: </strong> {{ $article->published_at->format('d/m/y h:ia') }}</p>
                            </div>

                            <div class="article-meta-info"
                                style="font-size: 13px; color: #888; padding: 8px 0; margin-bottom: 10px; border-bottom: 1px solid #eee;">
                                ✨ {{ $article->views ?? 0 }} views &nbsp; | &nbsp; 💬
                                {{ $article->comments->where('status', 1)->count() }} comments &nbsp; | &nbsp; Posted
                                ...
                                @auth
                                    <div class="msg_incontent mb-4">
                                        <form action="{{ route('article.comment', $article->id) }}" method="POST">
                                            @csrf
                                            <textarea name="content" cols="" rows="3" placeholder="Add a Comment........"
                                                required></textarea>
                                    ...
                                    @foreach($article->comments->where('status', 1)->sortByDesc('created_at') as $comment)
                                        <div class="comment-item mb-3 p-3 border-bottom"
                                            ...
                                            <div class="d-flex align-items-center mb-2">
                                                <img src="{{ $comment->user->profile_image }}" alt="User"
                                                    style="width: 30px; height: 30px; border-radius: 50%; margin-right: 10px;">
                                                <strong style="color: #729b0f;">{{ $comment->user->name }}</strong>
                                                <span
                                                    class="text-muted ml-2 small">{{ $comment->created_at->diffForHumans() }}</span>
                                            </div>
                    ...
                    <div class="rgt_articlelist">
                        ...
                        <ul>
                            @foreach($categories as $category_item)
                                <li>
                                    <a href="{{ route('article.category', $category_item->slug) }}">{{ $category_item->title }}
                                        <span>({{ $category_item->articles()->published()->count() }})</span></a>
                                </li>
                            @endforeach
                        </ul>
                    </div>
