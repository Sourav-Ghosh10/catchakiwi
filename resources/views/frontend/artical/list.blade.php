                        ...
                        @forelse($articles as $article)
                            <div class="search_restspan article_listpan">
                                <div class="row">
                                    ...
                                    @endif
                                    <div
                                        class="{{ $article->image ? 'col-lg-8 col-md-8' : 'col-lg-12 col-md-12' }} col-sm-12 results_rightdtls">
                                        <h4>{{ $article->title }}</h4>
                                        ...
                                        <div class="mobweb_txt">
                                            <ul>
                                                <li class="user"><img src="{{ asset('assets/images/user_icon.png') }}"
                                                        alt=""> <a
                                                        href="{{ url($country_name . '/profile/' . Crypt::encryptString($article->user->id)) }}">{{ $article->user->name }}</a>
                                                </li>
                                                <li><img src="{{ asset('assets/images/time_icon.png') }}" alt="">
                                                    {{ $article->published_at->format('d/m/y h:ia') }}</li>
                                                <li><img src="{{ asset('assets/images/view_icon.png') }}" alt="">
                                                    {{ $article->views }} views</li>
                                            </ul>
                                        </div>
                                        <p>{{ Str::limit(strip_tags($article->content), 180) }}</p>
                                        <a href="{{ route('article.details', $article->slug) }}"
                                            class="article_readmorebut">Read More</a>
                                    </div>
                                </div>
                            </div>
                        @empty
...
<div class="support_advertiser mobaddlist">
    <i class="fa fa-heart"></i>
    <span>Support our advertisers, catchakiwi exists because of them</span>
</div>
@foreach($sideData as $ad)
